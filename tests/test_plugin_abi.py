...
l,a=api(R/'build/host/melodic_dsp.so');i=a.create_instance(str(R/'modules/warpmrsample').encode(),b'{}');a.set_param(i,b'sample_path',str(wav).encode());assert get(a,i,'root_note')=='69';a.set_param(i,b'length_pct',b'150');assert abs(float(get(a,i,'length_ms'))-750)<1;a.set_param(i,b'length_ms',b'250');assert abs(float(get(a,i,'length_pct'))-50)<1;a.destroy_instance(i);print('melodic ABI ok')
